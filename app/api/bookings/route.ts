...
// Auth helper — one account, one cookie
async function getUser(req: NextRequest) {
  const token = req.cookies.get("tester_token")?.value;
  if (!token) return null;
  const sql = getSql();
  const [user] = await sql`SELECT id, email, name FROM testers WHERE auth_token = ${token}`;
  return user || null;
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get bookings where user is tester OR where user posted the job
    const bookings = await sql`
      SELECT b.*, o.app_url, o.app_type, o.description as job_description,
             t.name as tester_name, t.email as tester_email,
             CASE WHEN o.email = ${user.email} THEN true ELSE false END as is_owner
      FROM bookings b
      JOIN orders o ON o.id = b.order_id
      LEFT JOIN testers t ON t.id = b.tester_id
      WHERE b.tester_id = ${user.id} OR o.email = ${user.email}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [order] = await sql`SELECT id, app_url, email FROM orders WHERE id = ${order_id} AND email = ${user.email}`;
    ...
    await sendBookingEmail(
      ...
      `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px">
        ...
        <p style="font-size:14px;color:#555;margin:0 0 8px">A test session has been scheduled.</p>
